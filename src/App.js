...
export default function App() {
  ...
  const handleLogin = () => {
    ...
    if (found) {
      ...
      setLoginError('❌ إيميل أو كلمة مرور غلط!');
    }
  };
  ...
  const handleFingerprint = async () => {
    try {
      ...
      if (credential) setUser(USERS[0]);
    } catch (e) {
      alert('البصمة غير متوفرة على هذا الجهاز — استخدم كلمة المرور');
    }
  };
  ...
  const s = {
    ...
    input: { background: '#111827', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: '12px 16px', color: '#f1f5f9', fontFamily: 'Tajawal, sans-serif', fontSize: 15, outline: 'none', width: '100%' },
  };

  // LOGIN PAGE
  if (!user) {
    return (
      <div style={{ fontFamily: 'Tajawal, sans-serif', background: '#070b14', minHeight: '100vh', color: '#f1f5f9', direction: 'rtl', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ width: '100%', maxWidth: 420, padding: 24 }}>
          
          {/* Logo */}
          <div style={{ textAlign: 'center', marginBottom: 40 }}>
            <div style={{ width: 80, height: 80, background: 'linear-gradient(135deg, #38bdf8, #a78bfa)', borderRadius: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 40, margin: '0 auto 16px', boxShadow: '0 0 40px rgba(56,189,248,0.3)' }}>🏠</div>
            <h1 style={{ fontFamily: 'Cairo, sans-serif', fontSize: 28, fontWeight: 900, margin: 0, background: 'linear-gradient(135deg, #f1f5f9, #38bdf8)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Familia Alawar</h1>
            <p style={{ color: '#64748b', fontSize: 14, marginTop: 8 }}>منصة الأمان العائلي</p>
          </div>

          {/* Login Card */}
          <div style={{ background: '#0c1220', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 20, padding: 32 }}>
            <h2 style={{ fontFamily: 'Cairo, sans-serif', fontWeight: 700, fontSize: 18, marginBottom: 24, textAlign: 'center' }}>تسجيل الدخول</h2>
            
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                <label style={{ fontSize: 13, color: '#94a3b8', marginBottom: 6, display: 'block' }}>الإيميل</label>
                <input
                  type="email"
                  placeholder="rafat@familia-alawar.com"
              ...
              {loginError && (
                <div style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 10, padding: '10px 14px', color: '#f87171', fontSize: 13, textAlign: 'center' }}>
                  {loginError}
                </div>
              )}

              <button onClick={handleLogin} style={{ background: 'linear-gradient(135deg, #38bdf8, #0284c7)', border: 'none', color: '#070b14', padding: '14px', borderRadius: 12, cursor: 'pointer', fontFamily: 'Tajawal, sans-serif', fontSize: 16, fontWeight: 700, marginTop: 4 }}>
                دخول →
              </button>

              <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '4px 0' }}>
                <div style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.06)' }}></div>
                <span style={{ color: '#475569', fontSize: 13 }}>أو</span>
                <div style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.06)' }}></div>
              </div>

              <button onClick={handleFingerprint} style={{ background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', color: '#38bdf8', padding: '14px', borderRadius: 12, cursor: 'pointer', fontFamily: 'Tajawal, sans-serif', fontSize: 15, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                👆 الدخول ببصمة الإصبع
              </button>
            </div>
          </div>
  ...
  // MAIN APP
  return (
    <div style={{ fontFamily: 'Tajawal, sans-serif', background: '#070b14', minHeight: '100vh', color: '#f1f5f9', direction: 'rtl' }}>
      ...
      {sos && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#1a0a0a', border: '2px solid #ef4444', borderRadius: 20, padding: 40, textAlign: 'center', maxWidth: 400 }}>
            ...
            <p style={{ color: '#fca5a5', marginBottom: 24 }}>تم إرسال موقع {user.name} لجميع أفراد العائلة</p>
            <button onClick={() => setSos(false)} style={{ background: '#ef4444', color: 'white', border: 'none', padding: '12px 32px', borderRadius: 12, fontSize: 16, cursor: 'pointer', fontFamily: 'Tajawal, sans-serif' }}>إلغاء الطوارئ</button>
          </div>
        </div>
      )}

      {/* HEADER */}
      <div style={{ background: '#0c1220', borderBottom: '1px solid rgba(255,255,255,0.06)', padding: '0 24px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: '#64748b' }}>أهلاً {user.name} {user.emoji}</div>
          </div>
        </div>
        ...
        <button onClick={() => setUser(null)} style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', color: '#f87171', padding: '8px 16px', borderRadius: 8, cursor: 'pointer', fontFamily: 'Tajawal, sans-serif', fontSize: 13 }}>
          خروج 🚪
        </button>
      </div>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: 24 }}>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 20 }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                <div style={s.card}>
                  <div style={s.cardHeader}><span style={{ fontWeight: 700 }}>👨‍👩‍👧‍👦 أفراد العائلة</span></div>
                  {members.map(m => (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                <div style={{ ...s.card, flex: 1 }}>
                  <div style={s.cardHeader}><span style={{ fontWeight: 700 }}>💬 آخر الرسائل</span></div>
        ...
        {page === 'email' && (
          <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 20 }}>
            <div style={s.card}>
              <div style={s.cardHeader}><span style={{ fontWeight: 700 }}>📧 الإيميل</span></div>
              <div style={{ padding: 12 }}>
              ...
              {composing ? (
                <div>
                  ...
                  <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
                    ...
                    <button onClick={() => { setComposing(false); setNewEmail({ to: '', subject: '', body: '' }); }} style={{ background: 'linear-gradient(135deg, #38bdf8, #0284c7)', border: 'none', color: 'white', padding: '12px', borderRadius: 10, cursor: 'pointer', fontFamily: 'Tajawal, sans-serif', fontSize: 14, fontWeight: 700 }}>إرسال ✈️</button>
                  </div>
                </div>
              ) : selectedEmail ? (
                <div>
                  ...
                  <div style={{ padding: 20 }}>
                    ...
                    <p style={{ lineHeight: 1.8, color: '#94a3b8' }}>رسالة من {selectedEmail.from} بخصوص: {selectedEmail.subject}</p>
                  </div>
                </div>
              ) : (
                <div>
                  <div style={s.cardHeader}><span style={{ fontWeight: 700 }}>📥 صندوق الوارد</span></div>
                  {emails.map(e => (
                    <div key={e.id} onClick={() => setSelectedEmail(e)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '16px 20px', borderBottom: '1px solid rgba(255,255,255,0.04)', cursor: 'pointer' }}>
                      ...
                      {!e.read && <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#38bdf8' }}></div>}
                    </div>
                  ))}
                </div>
              )}
        ...
        {page === 'chat' && (
          <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 20 }}>
            <div style={s.card}>
              <div style={s.cardHeader}><span style={{ fontWeight: 700 }}>👨‍👩‍👧‍👦 العائلة</span></div>
              {members.map(m => (
            ...
            <div style={{ ...s.card, display: 'flex', flexDirection: 'column' }}>
              <div style={s.cardHeader}><span style={{ fontWeight: 700 }}>💬 مجموعة Familia Alawar</span></div>
              <div style={{ flex: 1, padding: 16, overflowY: 'auto', maxHeight: 400, display: 'flex', flexDirection: 'column', gap: 12 }}>
                {messages.map(m => (
                  <div key={m.id} style={{ display: 'flex', flexDirection: m.sender === user.name ? 'row-reverse' : 'row', gap: 8 }}>
                    <div style={{ maxWidth: '75%', background: m.sender === user.name ? 'rgba(56,189,248,0.1)' : '#1a2236', border: `1px solid ${m.sender === user.name ? 'rgba(56,189,248,0.2)' : 'rgba(255,255,255,0.06)'}`, borderRadius: 12, padding: '8px 12px', fontSize: 13 }}>
