...
function LikeComment({ item, user, color, t, onUpdate }) {
  ...
  const toggleLike = () => { const likes = item.likes || []; onUpdate({ ...item, likes: liked ? likes.filter(l => l !== user.name) : [...likes, user.name] }); };
  const addComment = () => {
    if (!commentText.trim()) return;
    onUpdate({ ...item, comments: [...(item.comments || []), { id: Date.now(), author: user.name, text: commentText, time: 'الآن' }] });
    setCommentText('');
  };
  return (
    <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '8px 14px' }}>
      <div style={{ display: 'flex', gap: 16, marginBottom: showComments ? 10 : 0 }}>
        <button onClick={toggleLike} style={{ background: 'transparent', border: 'none', color: liked ? color : 'rgba(255,255,255,0.5)', cursor: 'pointer', fontSize: 13, display: 'flex', alignItems: 'center', gap: 4 }}>{liked ? '❤️' : '🤍'} {item.likes?.length || 0} {t.likes}</button>
        <button onClick={() => setShowComments(!showComments)} style={{ background: 'transparent', border: 'none', color: 'rgba(255,255,255,0.5)', cursor: 'pointer', fontSize: 13, display: 'flex', alignItems: 'center', gap: 4 }}>💬 {item.comments?.length || 0} {t.comment}</button>
      </div>
      {showComments && (
        <div>
          {(item.comments || []).map((c, i) => (<div key={i} style={{ display: 'flex', gap: 8, padding: '6px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}><div style={{ fontFamily: 'Cairo, sans-serif', fontWeight: 700, fontSize: 12, color: color }}>{c.author}:</div><div style={{ fontSize: 13, color: 'rgba(255,255,255,0.8)' }}>{c.text}</div></div>))}
          <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
            <input value={commentText} onChange={e => setCommentText(e.target.value)} onKeyPress={e => e.key === 'Enter' && addComment()} placeholder={t.writeComment} style={{ flex: 1, background: '#444', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '8px 12px', color: '#fff', fontSize: 13, outline: 'none' }} />
...
function ReelItem({ reel, user, color, t, onUpdate }) {
  ...
  const togglePlay = () => { if (videoRef.current) { if (playing) { videoRef.current.pause(); setPlaying(false); } else { videoRef.current.play(); setPlaying(true); } } };
  return (
    <div style={{ position: 'relative', height: '100%', background: '#000', borderRadius: 16, overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', fontSize: 48, opacity: playing ? 0 : 0.8, pointerEvents: 'none' }}>▶️</div>
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: 16, background: 'linear-gradient(transparent, rgba(0,0,0,0.8))' }}>
        ...
        <button onClick={() => { const likes = reel.likes || []; const liked = likes.includes(user.name); onUpdate({ ...reel, likes: liked ? likes.filter(l => l !== user.name) : [...likes, user.name] }); }} style={{ background: 'transparent', border: 'none', color: reel.likes?.includes(user.name) ? color : '#fff', cursor: 'pointer', fontSize: 20 }}>
...
// SOS numbers
const SOS_NUMBERS = ['50663746666', '50660269374'];
...
export default function App() {
  ...
  const [posts, setPosts] = useState([{ id: 1, name: 'رأفت', emoji: '👨', text: 'أهلاً بالجميع! 🏠❤️', time: 'منذ ساعة', likes: [], comments: [] }]);
  ...
  const [trackingGPS, setTrackingGPS] = useState(() => localStorage.getItem('gpsActive') === 'true');
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, snap => {
      ...
      if (newMsgs.length > 0) {
        ...
        if (last.sender !== user?.name && Notification.permission === 'granted' && document.hidden) {
          new Notification('💬 رسالة جديدة — Familia Alawar', { body: `${last.sender}: ${last.text}` });
        }
      }
    });
    return unsub;
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem('gpsActive') === 'true') startGPS();
  // eslint-disable-next-line
  }, []);
  ...
  const sendSOSWhatsApp = (location) => {
    const mapsLink = location ? `https://maps.google.com/?q=${location[0]},${location[1]}` : 'موقع غير محدد';
    const sosMsg = encodeURIComponent(`🆘 طوارئ! ${user?.name || 'شخص'} يحتاج مساعدة!\n📍 الموقع: ${mapsLink}`);
    // إرسال للرقم الأول
    window.open(`https://wa.me/50663746666?text=${sosMsg}`, '_blank');
    // إرسال للرقم الثاني بعد ثانية
    setTimeout(() => {
      window.open(`https://wa.me/50660269374?text=${sosMsg}`, '_blank');
    }, 1000);
  };

  const activateSOS = () => {
    if (!trackingGPS) startGPS();
    setSosActive(true);
    sendSOSWhatsApp(myLocation);
    // إرسال كل 30 ثانية
  ...
  const sendMsg = async () => {
    ...
    await addDoc(collection(db, 'messages'), {
      sender: user.name, text: msg, room: chatRoom,
  ...
  const handlePhotoUpload = (e) => { Array.from(e.target.files).forEach(file => { const reader = new FileReader(); reader.onload = ev => setPhotos(prev => [...prev, { id: Date.now() + Math.random(), src: ev.target.result, uploader: user.name, time: 'الآن', likes: [], comments: [] }]); reader.readAsDataURL(file); }); };
  const handleVideoUpload = (e) => { Array.from(e.target.files).forEach(file => { setVideos(prev => [...prev, { id: Date.now() + Math.random(), src: URL.createObjectURL(file), uploader: user.name, time: 'الآن', likes: [], comments: [] }]); }); };
  const handleAlbumUpload = (e) => { Array.from(e.target.files).forEach(file => { const reader = new FileReader(); reader.onload = ev => setAlbumPhotos(prev => [...prev, { id: Date.now() + Math.random(), src: ev.target.result, uploader: user.name, time: 'الآن', likes: [], comments: [] }]); reader.readAsDataURL(file); }); };
  const handleMusicUpload = (e) => { Array.from(e.target.files).forEach(file => { setMusicList(prev => [...prev, { id: Date.now() + Math.random(), src: URL.createObjectURL(file), name: file.name.replace(/\.[^/.]+$/, ''), uploader: user.name }]); }); };
  const handleReelsUpload = (e) => { Array.from(e.target.files).forEach(file => { setReels(prev => [...prev, { id: Date.now() + Math.random(), src: URL.createObjectURL(file), uploader: user.name, uploaderEmoji: user.emoji, uploaderPhoto: profilePhoto, caption: '', likes: [], comments: [] }]); }); };
  ...
  const startRecording = async () => {
    try {
      ...
      recorder.onstop = async () => {
        ...
        await addDoc(collection(db, 'messages'), { sender: user.name, text: '', audioUrl: URL.createObjectURL(blob), room: chatRoom, to: chatRoom !== 'group' ? chatRoom : null, time: new Date().toLocaleTimeString('ar', { hour: '2-digit', minute: '2-digit' }), createdAt: serverTimestamp() });
  ...
  const handleChangePass = () => {
    ...
    user.password = newPass.new; setPassMsg('✅'); setNewPass({ current: '', new: '', confirm: '' });
  };
  ...
  const CH = (left, right) => (<div style={s.cardHeader}><span style={{ fontFamily: 'Cairo, sans-serif', fontWeight: 700 }}>{left}</span>{right}</div>);
    ...
    return (
      <div style={{ fontFamily: 'Tajawal, sans-serif', background: C.bg, minHeight: '100vh', color: '#fff', direction: dir, display: 'flex', flexDirection: 'column' }}>
        <div style={{ background: H, padding: '16px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <button onClick={() => setLang(lang === 'ar' ? 'es' : 'ar')} style={{ background: 'rgba(255,255,255,0.2)', border: 'none', color: '#fff', padding: '6px 14px', borderRadius: 20, cursor: 'pointer', fontFamily: 'Cairo, sans-serif', fontSize: 13, fontWeight: 700 }}>{lang === 'ar' ? '🇪🇸 Español' : '🇸🇾 عربي'}</button>
        </div>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px 24px' }}>
          ...
          <div style={{ width: '100%', maxWidth: 400, background: '#3d3d3d', borderRadius: 20, overflow: 'hidden' }}>
            <div style={{ background: H, padding: '16px 24px', textAlign: 'center' }}><h2 style={{ fontFamily: 'Cairo, sans-serif', fontWeight: 700, fontSize: 18, margin: 0 }}>{t.login}</h2></div>
  ...
  return (
    <div style={{ fontFamily: 'Tajawal, sans-serif', background: C.bg, minHeight: '100vh', color: '#fff', direction: dir, display: 'flex', flexDirection: 'column' }}>
      ...
      {sosActive && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.92)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#3d3d3d', border: `2px solid ${H}`, borderRadius: 20, padding: 40, textAlign: 'center', maxWidth: 400 }}>
            ...
            <p style={{ fontFamily: 'Tajawal, sans-serif', color: 'rgba(255,255,255,0.8)', marginBottom: 8 }}>{t.sentLocation}</p>
            <p style={{ fontFamily: 'Tajawal, sans-serif', color: 'rgba(255,255,255,0.5)', fontSize: 12, marginBottom: 24 }}>
              📲 تم إرسال موقعك لـ: 63746666 و 60269374
            </p>
            <button onClick={cancelSOS} style={{ background: H, color: 'white', border: 'none', padding: '12px 32px', borderRadius: 12, fontSize: 16, cursor: 'pointer', fontFamily: 'Cairo, sans-serif', fontWeight: 700 }}>{t.cancelSOS}</button>
          </div>
        </div>
      )}
            ...
            {notifications.slice(0, 10).map(n => (
              <div key={n.id} onClick={() => setNotifications(prev => prev.map(x => x.id === n.id ? { ...x, read: true } : x))}
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.6)' }}>{n.body}</div>
                </div>
      ...
      <div style={{ background: H, padding: '0 24px', height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <button onClick={() => setLang(lang === 'ar' ? 'es' : 'ar')} style={{ background: 'rgba(255,255,255,0.15)', border: 'none', color: '#fff', padding: '6px 12px', borderRadius: 20, cursor: 'pointer', fontFamily: 'Cairo, sans-serif', fontSize: 12, fontWeight: 700 }}>{lang === 'ar' ? '🇪🇸 ES' : '🇸🇾 AR'}</button>
      ...
      <div style={{ display: 'flex', flex: 1 }}>
        <div style={{ width: 190, background: '#2d2d2d', borderLeft: '1px solid rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column', padding: '12px 0', flexShrink: 0, overflowY: 'auto' }}>
          {NAV.map(n => (
            <button key={n.id} onClick={() => { setPage(n.id); setShowNotifPanel(false); if (n.id === 'sos') { activateSOS(); } }}
        ...
        <div style={{ flex: 1, padding: 24, overflowY: 'auto' }}>
          ...
          {page === 'home' && (
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
                {[{ label: t.family, value: '4', icon: '👨‍👩‍👧‍👦' }, { label: t.online, value: '3', icon: '🟢' }, { label: t.sharedPhotos, value: photos.length.toString(), icon: '📸' }, { label: t.newNotif, value: unreadCount.toString(), icon: '🔔' }].map((stat, i) => (
              ...
              <div style={{ ...s.card, marginBottom: 24 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)' }}>
                  {USERS.map((u, i) => { const days = daysUntil(u.birthday); return (<div key={i} style={{ padding: '16px 20px', borderLeft: i < 2 ? '1px solid rgba(255,255,255,0.06)' : 'none', textAlign: 'center' }}><div style={{ fontSize: 32, marginBottom: 8 }}>{u.emoji}</div><div style={{ fontFamily: 'Cairo, sans-serif', fontWeight: 700, fontSize: 14 }}>{u.name}</div><div style={{ background: days === 0 ? H : `${H}30`, borderRadius: 20, padding: '4px 12px', display: 'inline-block', fontFamily: 'Cairo, sans-serif', fontWeight: 700, fontSize: 13, color: days <= 7 ? '#ff6b6b' : '#fff', marginTop: 8 }}>{days === 0 ? t.today : `${days} ${t.days}`}</div></div>); })}
                </div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
                <div style={s.card}>
                  {CH('👨‍👩‍👧‍👦 ' + t.family)}
                  {MEMBERS.map((m, i) => (<div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 20px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}><div style={{ position: 'relative' }}><Avatar photo={m.name === user.name ? profilePhoto : null} emoji={m.emoji} size={40} /><div style={{ position: 'absolute', bottom: 0, right: 0, width: 10, height: 10, borderRadius: '50%', background: m.status === 'online' ? '#2ecc71' : '#f39c12', border: '2px solid #3d3d3d' }}></div></div><div style={{ flex: 1 }}><div style={{ fontFamily: 'Cairo, sans-serif', fontWeight: 700, fontSize: 14 }}>{m.name}</div><div style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)' }}>📍 {m.location}</div></div><div style={{ fontSize: 12, color: m.status === 'online' ? '#2ecc71' : '#f39c12' }}>{m.status === 'online' ? t.connected : t.away}</div></div>))}
                </div>
                <div style={s.card}>
                  {CH('📅 ' + t.upcomingEvents)}
                  {sortedEvents.slice(0, 4).map((e, i) => (<div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 16px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}><div style={{ width: 36, height: 36, borderRadius: 8, background: e.color + '30', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>{e.emoji}</div><div style={{ flex: 1 }}><div style={{ fontFamily: 'Cairo, sans-serif', fontWeight: 700, fontSize: 13 }}>{e.title}</div><div style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)' }}>{e.date}</div></div><div style={{ fontFamily: 'Cairo, sans-serif', fontWeight: 700, fontSize: 12, color: e.color }}>{daysUntil(e.date)} {t.days}</div></div>))}
                </div>
              </div>
              <div style={s.card}>
          ...
          {page === 'reels' && (
            <div>
              ...
              {reels.length === 0 ? (<div style={{ ...s.card, padding: 60, textAlign: 'center' }}><div style={{ fontSize: 70 }}>🎬</div><p style={{ color: 'rgba(255,255,255,0.6)' }}>{t.noReels}</p></div>) : (
                <div style={{ display: 'flex', gap: 20 }}>
                  <div style={{ width: 360, height: 640, flexShrink: 0 }}><ReelItem reel={reels[currentReel]} user={user} color={H} t={t} onUpdate={updated => setReels(prev => prev.map(r => r.id === updated.id ? updated : r))} /></div>
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 12 }}>
                    {reels.map((r, i) => (<div key={r.id} onClick={() => setCurrentReel(i)} style={{ ...s.card, display: 'flex', alignItems: 'center', gap: 12, padding: 12, cursor: 'pointer', border: i === currentReel ? `2px solid ${H}` : '1px solid rgba(255,255,255,0.08)' }}><video src={r.src} style={{ width: 80, height: 80, objectFit: 'cover', borderRadius: 8 }} /><div><div style={{ fontFamily: 'Cairo, sans-serif', fontWeight: 700, fontSize: 13 }}>{r.uploader}</div><div style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)' }}>❤️ {r.likes?.length || 0}</div></div></div>))}
                  </div>
                </div>
              )}
            </div>
          ...
          {page === 'photos' && (
            <div>
              ...
              {photos.length === 0 ? <div style={{ ...s.card, padding: 40, textAlign: 'center' }}><div style={{ fontSize: 60 }}>📸</div><p style={{ color: 'rgba(255,255,255,0.6)' }}>{t.noPhotos}</p></div>
                : <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: 16 }}>{photos.map(p => (<div key={p.id} style={s.card}><img src={p.src} alt="" style={{ width: '100%', height: 200, objectFit: 'cover' }} /><div style={{ padding: '10px 14px' }}><div style={{ fontFamily: 'Cairo, sans-serif', fontWeight: 700, fontSize: 13 }}>{p.uploader}</div></div><LikeComment item={p} user={user} color={H} t={t} onUpdate={updated => setPhotos(prev => prev.map(x => x.id === updated.id ? updated : x))} /></div>))}</div>}
            </div>
          )}
          ...
          {page === 'videos' && (
            <div>
              ...
              {videos.length === 0 ? <div style={{ ...s.card, padding: 40, textAlign: 'center' }}><div style={{ fontSize: 60 }}>🎥</div><p style={{ color: 'rgba(255,255,255,0.6)' }}>{t.noVideos}</p></div>
                : <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>{videos.map(v => (<div key={v.id} style={s.card}><video src={v.src} controls style={{ width: '100%', height: 200 }} /><div style={{ padding: '10px 14px' }}><div style={{ fontFamily: 'Cairo, sans-serif', fontWeight: 700, fontSize: 13 }}>{v.uploader}</div></div><LikeComment item={v} user={user} color={H} t={t} onUpdate={updated => setVideos(prev => prev.map(x => x.id === updated.id ? updated : x))} /></div>))}</div>}
            </div>
          )}
          ...
          {page === 'album' && (
            <div>
              ...
              {albumPhotos.length === 0 ? <div style={{ ...s.card, padding: 60, textAlign: 'center' }}><div style={{ fontSize: 70 }}>🖼️</div><p style={{ color: 'rgba(255,255,255,0.6)' }}>{lang === 'ar' ? 'الألبوم فارغ!' : '¡Álbum vacío!'}</p></div>
                : <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 16 }}>{albumPhotos.map(p => (<div key={p.id} style={s.card}><img src={p.src} alt="" style={{ width: '100%', height: 200, objectFit: 'cover' }} /><div style={{ padding: '10px 14px' }}><div style={{ fontFamily: 'Cairo, sans-serif', fontWeight: 700, fontSize: 13 }}>{p.uploader}</div></div><LikeComment item={p} user={user} color={H} t={t} onUpdate={updated => setAlbumPhotos(prev => prev.map(x => x.id === updated.id ? updated : x))} /></div>))}</div>}
            </div>
          )}
          ...
          {page === 'music' && (
            <div>
              ...
              {musicList.length === 0 ? <div style={{ ...s.card, padding: 60, textAlign: 'center' }}><div style={{ fontSize: 70 }}>🎵</div><p style={{ color: 'rgba(255,255,255,0.6)' }}>{lang === 'ar' ? 'لا توجد موسيقى!' : '¡No hay música!'}</p></div>
                : <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>{musicList.map(m => (<div key={m.id} style={{ ...s.card, padding: 16 }}><div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 12 }}><div style={{ width: 50, height: 50, borderRadius: 12, background: `${H}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24 }}>🎵</div><div><div style={{ fontFamily: 'Cairo, sans-serif', fontWeight: 700, fontSize: 15 }}>{m.name}</div><div style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)' }}>{m.uploader}</div></div></div><audio src={m.src} controls style={{ width: '100%' }} /></div>))}</div>}
            </div>
          )}
          ...
          {page === 'posts' && (
            <div>
              ...
              {posts.map((p, i) => (<div key={i} style={{ ...s.card, marginBottom: 12 }}><div style={{ padding: 20 }}><div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}><Avatar photo={p.photo} emoji={p.emoji} size={40} /><div><div style={{ fontFamily: 'Cairo, sans-serif', fontWeight: 700 }}>{p.name}</div><div style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)' }}>{p.time}</div></div></div><p style={{ fontFamily: 'Tajawal, sans-serif', fontSize: 15, lineHeight: 1.6, margin: 0 }}>{p.text}</p></div><LikeComment item={p} user={user} color={H} t={t} onUpdate={updated => setPosts(prev => prev.map(x => x.id === updated.id ? updated : x))} /></div>))}
            </div>
          )}
          ...
          {page === 'chat' && (
            <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr', gap: 16, height: 600 }}>
              <div style={{ ...s.card, display: 'flex', flexDirection: 'column' }}>
                ...
                {MEMBERS.filter(m => m.name !== user.name).map((m, i) => (
                  <div key={i} onClick={() => setChatRoom(m.name)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', cursor: 'pointer', background: chatRoom === m.name ? `${H}30` : 'transparent', borderBottom: '1px solid rgba(255,255,255,0.06)', borderRight: chatRoom === m.name ? `3px solid ${H}` : '3px solid transparent' }}>
                    <div style={{ position: 'relative' }}><Avatar photo={null} emoji={m.emoji} size={40} /><div style={{ position: 'absolute', bottom: 0, right: 0, width: 10, height: 10, borderRadius: '50%', background: m.status === 'online' ? '#2ecc71' : '#f39c12', border: '2px solid #3d3d3d' }}></div></div>
              ...
              <div style={{ ...s.card, display: 'flex', flexDirection: 'column' }}>
                ...
                {replyTo && (<div style={{ padding: '8px 16px', background: `${H}15`, borderTop: `1px solid ${H}30`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ fontSize: 12, color: 'rgba(255,255,255,0.7)' }}>↩️ {replyTo.text?.substring(0, 30)}</span><button onClick={() => setReplyTo(null)} style={{ background: 'transparent', border: 'none', color: '#fff', cursor: 'pointer', fontSize: 16 }}>✕</button></div>)}
                <div style={{ padding: '6px 16px', borderTop: '1px solid rgba(255,255,255,0.06)', display: 'flex', gap: 6 }}>
                  {['😊','😂','❤️','👍','🎉','😢','😮','🙏','👋','🔥'].map(emoji => (<button key={emoji} onClick={() => setMsg(prev => prev + emoji)} style={{ background: 'transparent', border: 'none', fontSize: 18, cursor: 'pointer', padding: '3px' }}>{emoji}</button>))}
                </div>
            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 20 }}>
                ...
                <div style={s.card}>
                  ...
                  {showAddEvent && (<div style={{ padding: 16, borderBottom: '1px solid rgba(255,255,255,0.06)' }}><div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}><input placeholder="..." value={newEvent.title} onChange={e => setNewEvent({ ...newEvent, title: e.target.value })} style={{ ...s.input, fontSize: 13, padding: '10px 14px' }} /><input type="date" value={newEvent.date} onChange={e => setNewEvent({ ...newEvent, date: e.target.value })} style={{ ...s.input, fontSize: 13, padding: '10px 14px' }} /><button onClick={addEvent} style={{ ...s.btn(H), padding: '10px', fontSize: 13 }}>✓</button></div></div>)}
                  <div style={{ maxHeight: 400, overflowY: 'auto' }}>
                    {sortedEvents.map((e, i) => (<div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}><div style={{ width: 40, height: 40, borderRadius: 10, background: e.color + '20', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0 }}>{e.emoji}</div><div style={{ flex: 1 }}><div style={{ fontFamily: 'Cairo, sans-serif', fontWeight: 700, fontSize: 13 }}>{e.title}</div><div style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)' }}>{e.date}</div></div><div style={{ textAlign: 'center' }}><div style={{ fontFamily: 'Cairo, sans-serif', fontWeight: 700, fontSize: 16, color: e.color }}>{daysUntil(e.date)}</div><div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)' }}>{t.days}</div></div></div>))}
                  </div>
                </div>
              </div>
            </div>
          ...
          {page === 'notifications' && (
            <div>
              ...
              <div style={s.card}>
                {CH(`${t.allNotifications} (${notifications.length})`, <button onClick={() => setNotifications(prev => prev.map(n => ({ ...n, read: true })))} style={{ background: `${H}20`, border: `1px solid ${H}40`, color: H, padding: '6px 14px', borderRadius: 8, cursor: 'pointer', fontSize: 13 }}>{t.readAll}</button>)}
                {notifications.map(n => (<div key={n.id} onClick={() => setNotifications(prev => prev.map(x => x.id === n.id ? { ...x, read: true } : x))} style={{ display: 'flex', gap: 14, padding: '14px 20px', borderBottom: '1px solid rgba(255,255,255,0.06)', background: n.read ? 'transparent' : `${H}10`, cursor: 'pointer' }}><div style={{ width: 44, height: 44, borderRadius: '50%', background: n.read ? '#444' : H, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, flexShrink: 0 }}>{n.icon}</div><div style={{ flex: 1 }}><div style={{ fontFamily: 'Cairo, sans-serif', fontWeight: n.read ? 400 : 700, fontSize: 14 }}>{n.title}</div><div style={{ fontSize: 13, color: 'rgba(255,255,255,0.7)' }}>{n.body}</div><div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)' }}>{n.time}</div></div>{!n.read && <div style={{ width: 10, height: 10, borderRadius: '50%', background: H, flexShrink: 0, marginTop: 6 }}></div>}</div>))}
              </div>
            </div>
          )}
            ...
            <div>
              ...
              <div style={{ ...s.card, marginBottom: 20 }}>
                ...
                <div style={{ padding: 24 }}>
                  {[{ icon: '📧', label: t.email, value: user.email }, { icon: '📱', label: t.phone, value: settingsPrivacy.showPhone ? (user.phone || '-') : '••••••••' }, { icon: '📍', label: t.location, value: user.location || '-' }, { icon: '🎂', label: lang === 'ar' ? 'تاريخ الميلاد' : 'Cumpleaños', value: settingsPrivacy.showBirthday ? (user.birthday || '-') : '••••••••' }].map((item, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '14px 0', borderBottom: i < 3 ? '1px solid rgba(255,255,255,0.06)' : 'none' }}>
                      ...
                      <div style={{ flex: 1 }}><div style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)' }}>{item.label}</div><div style={{ fontFamily: 'Cairo, sans-serif', fontWeight: 600, fontSize: 14 }}>{item.value}</div></div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ ...s.card, marginBottom: 20 }}>
                ...
                <div style={{ padding: 24, textAlign: 'center' }}>
                  <div style={{ position: 'relative', width: 100, height: 100, margin: '0 auto 16px' }}>
                    ...
                    <input type="file" accept="image/*" id="profilePhotoUpload" style={{ display: 'none' }} onChange={e => { const file = e.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = ev => setProfilePhoto(ev.target.result); reader.readAsDataURL(file); }} />
                  </div>
              ...
              <div style={{ ...s.card, marginBottom: 20 }}>
                ...
                <div style={{ padding: 20 }}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 12 }}>
                    {THEME_COLORS.map((c, i) => (<div key={i} onClick={() => setThemeColor(c.value)} style={{ textAlign: 'center', cursor: 'pointer' }}><div style={{ width: 44, height: 44, borderRadius: '50%', background: c.value, margin: '0 auto 6px', border: themeColor === c.value ? '3px solid #fff' : '3px solid transparent', boxShadow: themeColor === c.value ? `0 0 12px ${c.value}` : 'none' }} /><div style={{ fontSize: 11, color: 'rgba(255,255,255,0.6)' }}>{c.name}</div></div>))}
                  </div>
                </div>
              </div>
              <div style={{ ...s.card, marginBottom: 20 }}>
                ...
                <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
                  {[{ key: 'name', label: t.name, placeholder: user.name }, { key: 'phone', label: t.phone, placeholder: user.phone || '+506...' }, { key: 'location', label: t.location, placeholder: user.location || 'Costa Rica' }].map((item, i) => (
                    <div key={i}><label style={{ fontSize: 13, color: 'rgba(255,255,255,0.6)', marginBottom: 6, display: 'block' }}>{item.label}</label><input value={editProfile[item.key]} onChange={e => setEditProfile({ ...editProfile, [item.key]: e.target.value })} placeholder={item.placeholder} style={s.input} /></div>
                  ))}
                </div>
              </div>
              <div style={{ ...s.card, marginBottom: 20 }}>
                {CH('🔒 ' + t.privacy)}
                <div style={{ padding: 20 }}>
                  {[{ key: 'showLocation', label: t.showLocation, icon: '📍' }, { key: 'showBirthday', label: t.showBirthday, icon: '🎂' }, { key: 'showPhone', label: t.showPhone, icon: '📱' }].map((item, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: i < 2 ? '1px solid rgba(255,255,255,0.06)' : 'none' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}><span style={{ fontSize: 20 }}>{item.icon}</span><span style={{ fontSize: 14 }}>{item.label}</span></div>
              ...
              <div style={{ ...s.card, marginBottom: 20 }}>
                ...
                <div style={{ padding: 20 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div><div style={{ fontFamily: 'Cairo, sans-serif', fontWeight: 700, fontSize: 14 }}>{lang === 'ar' ? 'تتبع الموقع' : 'Rastreo'}</div><div style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)' }}>{trackingGPS ? t.gpsActive : t.gpsStopped}</div></div>
          ...
          {page === 'sos' && (
            <div>
              ...
              <div style={{ ...s.card, padding: 40, textAlign: 'center' }}>
                {myLocation && <div style={{ background: 'rgba(46,204,113,0.1)', borderRadius: 10, padding: '10px 20px', marginBottom: 16, color: '#2ecc71', fontSize: 13 }}>📍 {myLocation[0].toFixed(5)}, {myLocation[1].toFixed(5)}</div>}
                <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: 12, padding: '12px 20px', marginBottom: 24, fontSize: 13, color: 'rgba(255,255,255,0.7)' }}>
                  📲 سيتم إرسال موقعك تلقائياً عبر WhatsApp إلى:<br />
                  <strong style={{ color: '#2ecc71' }}>63746666</strong> و <strong style={{ color: '#2ecc71' }}>60269374</strong><br />
                  كل 30 ثانية
                </div>
                <p style={{ color: 'rgba(255,255,255,0.7)', marginBottom: 32, fontSize: 15 }}>{t.emergencyMsg}</p>
                <button onClick={activateSOS} style={{ width: 160, height: 160, borderRadius: '50%', background: 'radial-gradient(circle, #e74c3c, #c0392b)', border: '4px solid rgba(192,57,43,0.4)', color: 'white', fontSize: 32, fontWeight: 900, cursor: 'pointer', boxShadow: '0 0 40px rgba(192,57,43,0.5)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', margin: '0 auto', fontFamily: 'Cairo, sans-serif' }}>
                  <span>SOS</span>
                  <span style={{ fontSize: 11, fontWeight: 400, opacity: 0.8 }}>اضغط للطوارئ</span>
                </button>
                <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 13, marginTop: 24 }}>⚠️ استخدم فقط في الطوارئ الحقيقية</p>
              </div>
            </div>
          )}
